...
# preparing the text
corpus = [bohemia_ch1, bohemia_ch2, bohemia_ch3,
...
lda_bag_of_words_creator = LatentDirichletAllocation(
    learning_method='online', n_components=10)
lda_bag_of_words = lda_bag_of_words_creator.fit_transform(bag_of_words)

# creating the tf-idf LDA model
lda_tfidf_creator = LatentDirichletAllocation(
    learning_method='online', n_components=10)
...
for topic_id, topic in enumerate(lda_bag_of_words_creator.components_):
    ...
    message += " ".join([bag_of_words_creator.get_feature_names()[i]
...
for topic_id, topic in enumerate(lda_tfidf_creator.components_):
    message = "Topic #{}: ".format(topic_id + 1)
    message += " ".join([tfidf_creator.get_feature_names_out()[i]
